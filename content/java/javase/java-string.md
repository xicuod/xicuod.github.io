...
title: Java 字符串
---
在此之前，最好先了解[字符类型]({{< ref "java-data-types.md#数值类型-字符" >}})。

## 字符串的特性

* 不可变性：`final`，字符串对象的内容不可改变。
* JDK8及以前底层为字符数组`char[]`，JDK9起底层为字节数组`byte[]`。
...
- Java8 及以前：`StringBuilder`
    ...
    ```
- Java9 及以后：`StringConcatFactory` + `invokedynamic` 指令（[JVM 指令]({{< ref "jvm-bytecode-instruction.md" >}})，[Java 5 可变参数]()）
    ```java
