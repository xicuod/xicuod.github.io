...
```java
void add<E>(E e); /* Element 元素 */
void process<T>(T t); /* Type 类型 */
```
...
Java 的泛型是“伪泛型”。Java 5 引入泛型时，为了兼容老代码，只在老代码的基础上增加了**编译时类型检查机制**，并在编译通过时擦除类型实参，把类型实参替换为泛型的[边界类型](#有界泛型指定泛型的上界和下界) (bound type)。有上界的泛型会被替换为上界类型，没上界的会被直接替换为 `Object` 类型。之后访问泛型的数据时，编译器会把上界类型**强制转换**为类型实参。
...
对于一个泛型的方法参数，当不确定其类型实参时（例如在程序中，这个类型实参有时是 `Integer`，有时是 `String`），可用**泛型通配符** `?` 来替代类型实参，意思是“这个参数的类型实参是未知的”。
...
>
> 编译器实际上会给泛型通配符 `?` 创建一个**捕获类型**（如 `capture#1 of ?`），这个过程叫“**捕获转换**”。在方法上下文中，捕获类型会被当成类型实参来用。捕获类型是临时创建的，在代码中只能将捕获类型的数据安全地赋值给它的[上界类型](#有界泛型指定泛型的上界和下界)（或任何上界的父类型）的变量。如果没有设置上界类型，相当于上界是 `Object`，因为所有的引用类型都是 `Object` 的子类，捕获类型也不例外。
> 
> 捕获类型只是一个用来表示“未知的类型实参”的“占位把戏”：它只是“捕获”了未知的类型实参，本质上还是未知的（就像数学上把未知数设为 $x$）。因此，为了保证类型安全，编译器不会允许你在捕获类型的引用上写数据（除了引用类型通用的 `null`）。
...
当泛型的取值范围确定，但泛型的类型实参不确定时，可将泛型声明为**有界泛型**。有界泛型是[多态]({{% sref "java-polymorphism" %}})的典型应用。

> [!note] 泛型通配符
>
> `? extends` 和 `? super` 是“有界通配符”，`?` 是“无界通配符”。无界通配符 `?` 相当于 `? extends Object`。
...
**PECS 原则** (producer-extends, consumer-super, 生产者使用 `extends` 与消费者使用 `super` 原则) 是在编译时检查两个方法参数的类型实参是否符合**生产-消费关系**。PECS 原则是有界泛型的典型应用。

- `? extends T` 使生产者提供 `T` 或 `T` 以下的引用，保证用 `T` 变量可从生产者安全读取
- `? super T` 使消费者接收 `T` 或 `T` 以上的引用，保证可安全写入 `T` 引用到消费者
...
> [!note] PECS 原则
>
> PECS 原则是为了在保证类型安全的前提下尽可能提高生产-消费方法的灵活性。因为 Java 的泛型是[不变的](#泛型的不变性-invariance)。这样就会导致像 `printNumbers(List<Number> list)` 这样的方法只能处理父类列表，不能处理子类列表，缺乏灵活性。为此必须要引入有界泛型，让生产者的泛型有上界（灵活读），消费者的泛型有下界（灵活写）。
