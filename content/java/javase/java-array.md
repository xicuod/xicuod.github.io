...
* 每个数组都是一个对象，在运行时通过一个JVM动态生成的数组类实例化。
	...
	* JVM动态生成的数组类没有重写`Object`的`toString`方法，要打印对业务更有用的信息，使用`Arrays.toString(Object[] a)`或它的基础类型数组参数的重载。
...
多维数组可以看成是数组的数组，比如二维数组就是一个特殊的一维数组，其每一个元素都是一个一维数组。二维数组 `int a[][] = new int[2][5];` 可以看成一个两行五列的[矩阵](../../数据结构与算法/数据结构基础/串、数组、矩阵和广义表/矩阵%20Matrix.md)。
